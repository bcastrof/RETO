...
(USER IN USUARIOS.USUARIO%TYPE, PASS IN USUARIOS.PASSWORD%TYPE, CAT OUT TRABAJADORES.CATEGORIA%TYPE, IDU OUT USUARIOS.USUARIO%TYPE,
IDT OUT TRABAJADORES.ID%TYPE, C OUT SYS_REFCURSOR)
AS
...
ELSE 
SELECT T.ID, T.CATEGORIA, U.USUARIO INTO IDT, CAT, IDU
FROM TRABAJADORES T, (SELECT USUARIO FROM USUARIOS WHERE USUARIO=USER AND PASSWORD=PASS) AS U
